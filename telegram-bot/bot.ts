...
app.post('/send-code', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // ✅ ИСПРАВЛЕНО: Короткий payload для кнопки подтверждения
    const shortPayload = {
      d: draftId,  // Только draftId
    };

    const confirmPayload = Buffer.from(
      JSON.stringify(shortPayload)
    ).toString('base64');

    console.log(`📤 Payload для кнопки: ${JSON.stringify(shortPayload)}`);
    console.log(`📤 Encoded payload: ${confirmPayload}`);
    console.log(`📤 Payload длина: ${confirmPayload.length} символов`);

    if (confirmPayload.length > 64) {
      console.error(`⚠️ ПРЕДУПРЕЖДЕНИЕ: Callback payload слишком длинный (${confirmPayload.length} > 64)!`);
    }
...
bot.onText(/\/start (.+)/, async (msg, match) => {
  ...
  console.log(`🔗 [/start+payload] Payload длина: ${payload.length} символов`);

  try {
    // Декодируем payload
    const decoded = JSON.parse(Buffer.from(payload, 'base64').toString());
    console.log(`🔗 [/start+payload] Decoded:`, decoded);
    
    // ✅ ИСПРАВЛЕНО: Поддержка короткого формата
    const draftId = decoded.d || decoded.draftId;  // Поддерживаем оба формата
    ...
    if (!email) {
      throw new Error('Email not found in draft');
    }

    console.log(`📧 [/start+payload] Email from DB: ${email}`);
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('confirm_')) {
    ...
    try {
      ...
      // Декодируем payload
      const decoded = JSON.parse(Buffer.from(payload, 'base64').toString());
      console.log(`📩 [callback_query] Decoded:`, decoded);
      ...
      // Получаем email из БД если его нет в payload
      let email = decoded.e || decoded.email;
      
      if (!email) {
        console.log(`📩 [callback_query] Email отсутствует в payload, получаем из БД...`);
        ...
        console.log(`📩 [callback_query] Email from DB: ${email}`);
      }
