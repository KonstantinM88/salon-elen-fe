...
-- 3) Заполнить slot для существующих записей
UPDATE "Appointment"
...
-- 5) Триггер для поддержания slot
...
-- 6) Предохранитель: если есть пересечения у активных статусов — прервать миграцию
DO $$
BEGIN
  IF EXISTS (
    WITH a AS (
...
-- 7) Само EXCLUDE-ограничение — только по колонкам (никаких выражений/IMMUTABLE-функций)
ALTER TABLE "Appointment"
