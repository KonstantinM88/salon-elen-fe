...
async function ensureServices(): Promise<void> {
  const services: ReadonlyArray<{
    ...
    { slug: "haircut",   name: "Стрижка",  durationMin: 60, priceCents: 4500, isActive: true },
    { slug: "manicure",  name: "Маникюр",  durationMin: 60, priceCents: 3500, isActive: true },
    { slug: "makeup",    name: "Макияж",   durationMin: 90, priceCents: 6500, isActive: true },
  ];

  for (const s of services) {
    await prisma.service.upsert({
      where:  { slug: s.slug },
      update: { name: s.name, durationMin: s.durationMin, priceCents: s.priceCents, isActive: s.isActive },
      create: { slug: s.slug, name: s.name, durationMin: s.durationMin, priceCents: s.priceCents, isActive: s.isActive },
    });
  }
}
