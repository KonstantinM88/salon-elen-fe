...
export async function listServices(args: Args) {
  ...
  // Exclude orphaned children: if a service has a parentId but the parent
  // is missing from `byId`, that means the parent is inactive/archived.
  // In that case the child should NOT appear in the catalog either — otherwise
  // deactivating a parent category leaves its children "hanging" as if they
  // were top-level services.
  const visible = all.filter((row) => {
    if (!row.parentId) return true; // top-level — keep
    // Walk up to verify the WHOLE ancestor chain is active.
    let cursor: RawServiceRow | undefined = row as RawServiceRow;
    const seen = new Set<string>();
    while (cursor?.parentId) {
      if (seen.has(cursor.id)) break; // safety against cycles
      seen.add(cursor.id);
      const parent = byId.get(cursor.parentId);
      if (!parent) return false; // ancestor inactive/archived — hide this row
      cursor = parent;
    }
    return true;
  });
  ...
  const bookable: ServiceItem[] = visible
