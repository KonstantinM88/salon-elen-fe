...
/**
 * Matches clicks on "❓ Частые вопросы" / "❓ FAQ about PMU" / "❓ Häufige Fragen"
 * inside the technique-details view.
 */
export function isKnowledgePmuFaqIntent(
  text: string,
  locale?: string,
): boolean {
  const value = text.toLowerCase().replace(/ё/g, 'е').trim();
  ...
  if (normalized === 'ru') {
    return (
      value.includes('частые вопросы') ||
      value.includes('частые вопрос') ||
      value.includes('часто задаваемые') ||
      value.includes('faq') ||
      value.includes('вопросы и ответы')
    );
  }
  ...
  if (normalized === 'en') {
    return (
      value.includes('faq') ||
      value.includes('common questions') ||
      value.includes('frequently asked')
    );
  }

  return (
    value.includes('häufige fragen') ||
    value.includes('haufige fragen') ||
    value.includes('haeufige fragen') ||
    value.includes('faq') ||
    value.includes('häufig gestellte')
  );
}

/**
 * Matches clicks on "📋 Как подготовиться" / "📋 How to prepare" /
 * "📋 So bereiten Sie sich vor" inside the technique-details view.
 */
export function isKnowledgePmuPreparationIntent(
  text: string,
  locale?: string,
): boolean {
  const value = text.toLowerCase().replace(/ё/g, 'е').trim();
  ...
  if (normalized === 'ru') {
    return (
      value.includes('как подготовиться') ||
      value.includes('подготовка') ||
      value.includes('что нужно сделать перед') ||
      value.includes('перед процедурой')
    );
  }

  if (normalized === 'en') {
    return (
      value.includes('how to prepare') ||
      value.includes('preparation') ||
      value.includes('before the appointment') ||
      value.includes('before procedure')
    );
  }

  return (
    value.includes('vorbereit') ||
    value.includes('so bereiten') ||
    value.includes('wie soll ich mich') ||
    value.includes('vor dem termin')
  );
}
...
/**
 * Builds a rich PMU FAQ text from the existing PERMANENT_HALLE.pmu.faq dataset.
 * Used when the user clicks "❓ Частые вопросы" / "❓ FAQ" / "❓ Häufige Fragen"
 * inside a technique-details view.
 */
export function buildKnowledgePmuFaqText(
  locale: AssistantLocale | undefined,
): string {
  const lang = localeToLang(locale);
  const faq = PERMANENT_HALLE.pmu.faq[lang];

  if (lang === 'RU') {
    return [
      '**Частые вопросы о Permanent Make-up** 🌸',
      '',
      `❔ **Что такое PMU?**\n${faq.what_is}`,
      '',
      `❔ **Сколько держится?**\n${faq.duration}`,
      ...
      `❔ **Это больно?**\n${faq.pain}`,
      '',
      `❔ **Будет ли смотреться натурально?**\n${faq.natural}`,
      '',
      `❔ **Как проходит процедура?**\n${faq.process}`,
      '',
      `❔ **Нужна ли коррекция?**\n${faq.touch_up}`,
      '',
      `❔ **У меня уже есть старый перманент — можно?**\n${faq.old_pmu}`,
      '',
      '[option] 🌿 Подробно о заживлении [/option]',
      '[option] 📋 Как подготовиться [/option]',
      '[option] 📅 Подобрать время и записаться [/option]',
    ].join('\n');
  }
  ...
  if (lang === 'EN') {
    return [
      ...
      '',
      `❔ **I already have old PMU — is it OK?**\n${faq.old_pmu}`,
      '',
      '[option] 🌿 Healing process in detail [/option]',
      '[option] 📋 How to prepare [/option]',
      '[option] 📅 Pick time and book [/option]',
    ].join('\n');
  }
  ...
  return [
    ...
    '',
    `❔ **Ich habe schon altes PMU — geht das?**\n${faq.old_pmu}`,
    '',
    '[option] 🌿 Heilungsverlauf im Detail [/option]',
    '[option] 📋 So bereiten Sie sich vor [/option]',
    '[option] 📅 Zeit finden und buchen [/option]',
  ].join('\n');
}

/**
 * Builds a preparation guide (general + zone-specific) using
 * PERMANENT_HALLE.pmu.preparation. Optional technique narrows the zone tips.
 */
export function buildKnowledgePmuPreparationText(
  locale: AssistantLocale | undefined,
  technique?: KnowledgePmuTechnique,
): string {
  const lang = localeToLang(locale);
  const prep = PERMANENT_HALLE.pmu.preparation[lang];

  const zoneKey: 'brows' | 'lips' | 'eyes' | null =
    technique === 'powder_brows' || technique === 'hairstroke_brows'
      ? 'brows'
      : technique === 'aquarell_lips' || technique === 'lips_3d'
        ? 'lips'
        : technique === 'lashline' || technique === 'upper_lower'
          ? 'eyes'
          : null;

  const general = prep.general.map((line: string) => `• ${line}`).join('\n');
  const zone = zoneKey ? prep[zoneKey].map((line: string) => `• ${line}`).join('\n') : '';

  if (lang === 'RU') {
    const zoneTitle =
      zoneKey === 'brows'
        ? '**Подготовка зоны бровей:**'
        : zoneKey === 'lips'
          ? '**Подготовка зоны губ:**'
          : zoneKey === 'eyes'
            ...
            : '';
    const blocks = [
      '**Как подготовиться к PMU** 🌸',
      '',
      '**Общие рекомендации:**',
      general,
    ];
    if (zone) blocks.push('', zoneTitle, zone);
    blocks.push(
      '',
      '_В день процедуры приходите отдохнувшей, поешьте как обычно. Всё остальное согласуем на месте._',
      '',
  ...
  if (lang === 'EN') {
    const zoneTitle =
      zoneKey === 'brows'
        ? '**Brow zone preparation:**'
        : zoneKey === 'lips'
          ? '**Lip zone preparation:**'
          : zoneKey === 'eyes'
            ? '**Eye zone preparation:**'
            : '';
    const blocks = [
      '**How to prepare for PMU** 🌸',
      '',
      '**General tips:**',
      general,
    ];
    ...
    blocks.push(
      '',
      '_On the day, come rested and eat normally. We plan everything else on the spot._',
      '',
      '[option] ❓ FAQ about PMU [/option]',
      '[option] 🌿 Healing process in detail [/option]',
      '[option] 📅 Pick time and book [/option]',
    );
    return blocks.join('\n');
  ...
  const zoneTitle =
    zoneKey === 'brows'
      ? '**Vorbereitung Brauen-Zone:**'
      : zoneKey === 'lips'
        ? '**Vorbereitung Lippen-Zone:**'
        : zoneKey === 'eyes'
          ? '**Vorbereitung Augen-Zone:**'
          : '';
  const blocks = [
    '**So bereiten Sie sich auf PMU vor** 🌸',
    '',
    '**Allgemeine Hinweise:**',
    general,
  ];
  ...
  blocks.push(
    '',
    '_Am Tag der Behandlung: ausgeruht und normal gegessen kommen. Alles Weitere besprechen wir vor Ort._',
    '',
...
export function buildKnowledgePmuTechniqueDetailsText(
  ...
  // ── per-technique "narrative" content ────────────────────────────────
  type Narrative = {
    title: string;
    intro: string;
    idealFor: string;
  ...
  const ruN: Record<KnowledgePmuTechnique, Narrative> = {
    powder_brows: {
      title: '💄 Пудровые брови (Powder Brows)',
      intro:
        'Это мягкое пудровое напыление пигмента: брови выглядят оформленно, словно после лёгкого макияжа, без резких границ и тёмных линий.',
      idealFor:
        '🌸 Подходит, если хотите натуральный ежедневный эффект «уже с макияжем», ровную форму и аккуратный вид без чёткой прорисовки волосков.',
      process:
        '📋 Этапы: консультация и выбор формы → точная вырисовка эскиза (вы видите и одобряете) → пигментация → подробные рекомендации по уходу.',
      duration: '⏱️ Длительность процедуры: ~120 мин. Держится **1,5–3 года** в зависимости от типа кожи, мягко светлеет со временем.',
      price: '💰 Цена: **350 €**. Коррекция через 4–8 недель оплачивается отдельно (по прайсу).',
      healing:
        '🌿 Заживление ~4 недели. В первые дни цвет ярче и насыщеннее, к концу месяца оттенок становится мягким и натуральным. Возможно лёгкое шелушение в неделю 1.',
      aftercare:
        '✨ В период заживления: не тереть, не снимать корочки, избегать сауны/солярия/яркого солнца, не наносить макияж на зону.',
      compareLabel: '🌟 Сравнить с Волосковой техникой',
    },
    hairstroke_brows: {
      title: '💄 Волосковая техника (Hairstroke Brows)',
      intro:
        'Прорисовка отдельных «волосков» пигментом — даёт чёткую, структурную текстуру, имитирующую естественный рост брови.',
      idealFor:
        '🌸 Подходит, если хотите выраженный, более структурный результат с видимыми «волосками» — особенно красиво смотрится на брови с редкой плотностью.',
      process:
        '📋 Этапы: консультация → детальный эскиз и согласование формы → волосковая прорисовка пигментом → рекомендации по уходу.',
      duration: '⏱️ Длительность процедуры: ~180 мин. Держится **1,5–3 года**, мягко светлеет.',
      price: '💰 Цена: **450 €**. Коррекция через 4–8 недель — отдельно.',
      healing:
        '🌿 Заживление ~4 недели. Сначала «волоски» выглядят чётче и темнее, после заживления — мягче и натуральнее. Возможно лёгкое шелушение в первую неделю.',
      aftercare:
        '✨ Уход тот же: не трогать, не снимать корочки, защищать от солнца и тепла в период заживления.',
      compareLabel: '💖 Сравнить с Пудровыми бровями',
    },
    aquarell_lips: {
      title: '💋 Акварельные губы (Aquarell Lips)',
      intro:
        'Лёгкая, прозрачная пигментация без жёсткого контура — даёт мягкий ровный оттенок, как будто вы только что освежили цвет губ изнутри.',
      idealFor:
        '🌸 Идеально для естественного результата на каждый день, выравнивания цвета бледных губ или маскировки лёгкой асимметрии без эффекта помады.',
      process:
        '📋 Этапы: консультация и подбор оттенка под кожу → эскиз и одобрение → акварельная пигментация → пост-уход.',
      duration: '⏱️ Длительность процедуры: ~130 мин. Держится **1,5–3 года**, мягко светлеет.',
      price: '💰 Цена: **380 €**. Коррекция через 4–8 недель — отдельно.',
      healing:
        '🌿 Заживление ~4 недели. В первые дни возможны небольшой отёк и сухость губ, цвет сначала ярче. К концу месяца — мягкий, естественный финальный оттенок.',
      aftercare:
        '✨ Хорошее увлажнение губ обязательно, не сдирать корочки, избегать острой/горячей еды в первые дни.',
      compareLabel: '💄 Сравнить с 3D губами',
    },
    lips_3d: {
      title: '💋 3D Губы (3D Lips)',
      intro:
        'Более насыщенная пигментация с проработкой объёма — губы выглядят полнее и выразительнее, цвет ярче и устойчивее.',
      idealFor:
        '🌸 Подходит, если хочется заметный, «накрашенный» эффект на каждый день, более яркий цвет и визуальный объём.',
      process:
        '📋 Этапы: консультация и подбор насыщенного оттенка → детальный эскиз → 3D-пигментация → пост-уход.',
      duration: '⏱️ Длительность процедуры: ~180 мин. Держится **1,5–3 года**.',
      price: '💰 Цена: **420 €**. Коррекция через 4–8 недель — отдельно.',
      healing:
        '🌿 Заживление ~4 недели. В первые дни возможна отёчность и сухость, цвет более интенсивный. К 3–4 неделе — финальный, более естественный тон.',
      aftercare:
        '✨ Увлажнение губ, аккуратная еда первые дни, без сауны и солнца до полного заживления.',
      compareLabel: '💋 Сравнить с Акварельными губами',
    },
    lashline: {
      title: '👁 Межресничка',
      intro:
        'Деликатное заполнение пространства между ресницами: взгляд становится более выразительным, ресницы выглядят гуще, без эффекта подводки.',
      idealFor:
        '🌸 Подходит, если хочется освежить взгляд без видимой стрелки и без необходимости подводить глаза каждый день.',
      process:
        '📋 Этапы: консультация → подбор линии и оттенка → деликатная пигментация межресничного пространства → рекомендации.',
      duration: '⏱️ Длительность процедуры: ~60 мин. Держится **1,5–3 года**.',
      price: '💰 Цена: **130 €**. Вариант верх+низ — **150 €**. Коррекция — отдельно.',
      healing:
        '🌿 Заживление ~3–4 недели. 1–2 дня возможны небольшая чувствительность век и лёгкая отёчность, кратковременная слезоточивость.',
      aftercare:
        '✨ Не тереть глаза, без свежих процедур ресниц (наращивание, ламинирование) до заживления.',
      compareLabel: '👁 Сравнить с верх+низ (150 €)',
    },
    upper_lower: {
      title: '👁 Межресничка верх + низ',
      intro:
        'Деликатная пигментация межресничного пространства на обоих веках — даёт более заметный результат и выразительный взгляд, при этом без подводки.',
      idealFor:
        '🌸 Подходит, если хочется максимально освежить и оживить взгляд без ежедневного макияжа.',
      process:
        '📋 Этапы: консультация → подбор линии и оттенка → пигментация верхней + нижней межреснички → рекомендации.',
      duration: '⏱️ Длительность процедуры: ~75 мин. Держится **1,5–3 года**.',
      price: '💰 Цена: **150 €**. Только верх — **130 €**. Коррекция — отдельно.',
      healing:
        '🌿 Заживление ~3–4 недели. 1–2 дня возможна лёгкая чувствительность век и небольшая отёчность.',
      aftercare:
        '✨ Не тереть глаза, без свежих процедур ресниц до полного заживления, защищать от прямого солнца.',
      compareLabel: '👁 Сравнить с обычной межресничкой (130 €)',
    },
  };

  const enN: Record<KnowledgePmuTechnique, Narrative> = {
    powder_brows: {
      title: '💄 Powder Brows',
      intro:
        'A soft powder-shading pigmentation: brows look polished, as if lightly made up, without harsh edges.',
      idealFor:
        '🌸 Best if you want a natural everyday "wake up ready" look — soft shape, no visible hair strokes.',
      process:
        '📋 Steps: consultation & shape selection → precise pre-drawing (you approve) → pigmentation → aftercare guidance.',
      duration: '⏱️ Session: ~120 min. Lasts **1.5–3 years** depending on skin type, fading softly.',
      price: '💰 Price: **€350**. Touch-up after 4–8 weeks is billed separately.',
      healing:
        '🌿 Healing ~4 weeks. Color looks stronger in the first days, softens to a natural tone by week 4. Light flaking in week 1 is possible.',
      aftercare:
        '✨ During healing: do not rub or pick, avoid sauna/tanning/strong sun, no makeup on the area.',
      compareLabel: '🌟 Compare with Hairstroke Brows',
    },
    hairstroke_brows: {
      title: '💄 Hairstroke Brows',
      intro:
        'Individual hair-stroke pigmentation — a defined, structured texture that mimics natural brow hairs.',
      idealFor:
        '🌸 Best if you want a more noticeable, structured result with visible "hairs" — beautiful on sparse brows.',
      process:
        '📋 Steps: consultation → detailed pre-drawing & shape approval → hair-stroke pigmentation → aftercare.',
      duration: '⏱️ Session: ~180 min. Lasts **1.5–3 years**, fading softly.',
      price: '💰 Price: **€450**. Touch-up after 4–8 weeks is billed separately.',
      healing:
        '🌿 Healing ~4 weeks. Strokes appear sharper/darker at first, soften by week 4. Light flaking in week 1 is possible.',
      aftercare:
        '✨ Same care: do not touch, do not pick, protect from sun and heat during healing.',
      compareLabel: '💖 Compare with Powder Brows',
    },
    aquarell_lips: {
      title: '💋 Aquarelle Lips',
      intro:
        'Light, translucent pigmentation without a hard outline — gives a soft even tint, like a freshly refreshed natural lip color.',
      idealFor:
        '🌸 Perfect for an everyday natural result, evening out pale lips or masking mild asymmetry without a lipstick effect.',
      process:
        '📋 Steps: consultation & shade selection → pre-drawing & approval → aquarelle pigmentation → aftercare.',
      duration: '⏱️ Session: ~130 min. Lasts **1.5–3 years**, fading softly.',
      price: '💰 Price: **€380**. Touch-up after 4–8 weeks — separate.',
      healing:
        '🌿 Healing ~4 weeks. Slight swelling and dryness possible in the first days, color brighter at first. Final soft tone by week 4.',
      aftercare:
        '✨ Keep lips well moisturized, do not pick scabs, avoid spicy/hot food in the first days.',
      compareLabel: '💄 Compare with 3D Lips',
    },
    lips_3d: {
      title: '💋 3D Lips',
      intro:
        'A more saturated pigmentation with visual volume — lips appear fuller and more expressive, color is brighter and more stable.',
      idealFor:
        '🌸 Best if you want a noticeable "made up" everyday effect, richer color and added visual volume.',
      process:
        '📋 Steps: consultation & saturated shade selection → detailed pre-drawing → 3D pigmentation → aftercare.',
      duration: '⏱️ Session: ~180 min. Lasts **1.5–3 years**.',
      price: '💰 Price: **€420**. Touch-up after 4–8 weeks — separate.',
      healing:
        '🌿 Healing ~4 weeks. Swelling and dryness possible in the first days, color more intense at first. Final tone by week 3–4.',
      aftercare:
        '✨ Moisturize lips, careful eating in the first days, no sauna and sun until fully healed.',
      compareLabel: '💋 Compare with Aquarelle Lips',
    },
    lashline: {
      title: '👁 Lash line',
      intro:
        'A delicate fill between lashes: the gaze becomes more expressive, lashes look fuller — without an eyeliner effect.',
      idealFor:
        '🌸 Best if you want a refreshed gaze without a visible eyeliner and without daily eye makeup.',
      process:
        '📋 Steps: consultation → line & shade selection → delicate inter-lash pigmentation → aftercare guidance.',
      duration: '⏱️ Session: ~60 min. Lasts **1.5–3 years**.',
      price: '💰 Price: **€130**. Upper+lower option — **€150**. Touch-up separate.',
      healing:
        '🌿 Healing ~3–4 weeks. 1–2 days possible mild eyelid sensitivity and slight swelling, short-term watery eyes.',
      aftercare:
        '✨ Do not rub eyes, no fresh lash treatments (extensions, lifting) until healed.',
      compareLabel: '👁 Compare with Upper+Lower (€150)',
    },
    upper_lower: {
      title: '👁 Upper + Lower Lash line',
      intro:
        'Delicate pigmentation of the inter-lash space on both lids — a more pronounced result and expressive gaze, still without eyeliner.',
      idealFor:
        '🌸 Best if you want to fully refresh and brighten the gaze without any daily eye makeup.',
      process:
        '📋 Steps: consultation → line & shade selection → upper + lower pigmentation → aftercare.',
      duration: '⏱️ Session: ~75 min. Lasts **1.5–3 years**.',
      price: '💰 Price: **€150**. Upper only — **€130**. Touch-up separate.',
      healing:
        '🌿 Healing ~3–4 weeks. 1–2 days mild eyelid sensitivity and slight swelling possible.',
      aftercare:
        '✨ Do not rub eyes, no fresh lash treatments until fully healed, protect from direct sun.',
      compareLabel: '👁 Compare with lash line only (€130)',
    },
  };

  const deN: Record<KnowledgePmuTechnique, Narrative> = {
    powder_brows: {
      title: '💄 Powder Brows',
      intro:
        'Weicher Puder-Effekt: Augenbrauen wirken gepflegt wie nach leichtem Make-up, ohne harte Kanten oder dunkle Linien.',
      idealFor:
        '🌸 Ideal, wenn Sie einen natürlichen Alltags-Look möchten — weiche Form, keine sichtbaren Härchen.',
      process:
        '📋 Ablauf: Beratung & Formauswahl → präzise Vorzeichnung (Sie geben Ihr OK) → Pigmentierung → Pflegehinweise.',
      duration: '⏱️ Dauer: ~120 Min. Hält **1,5–3 Jahre** je nach Hauttyp, verblasst sanft.',
      price: '💰 Preis: **350 €**. Nachbehandlung nach 4–8 Wochen separat.',
      healing:
        '🌿 Heilung ~4 Wochen. Farbe wirkt anfangs kräftiger, wird zu Woche 4 weicher und natürlicher. Leichte Schuppung in Woche 1 möglich.',
      aftercare:
        '✨ In der Heilphase: nicht reiben, keine Schüppchen entfernen, Sauna/Solarium/starke Sonne meiden.',
      compareLabel: '🌟 Mit Hairstroke Brows vergleichen',
    },
    hairstroke_brows: {
      title: '💄 Hairstroke Brows',
      intro:
        'Pigmentierung einzelner "Härchen" — eine klare, strukturierte Textur, die natürlichen Augenbrauenwuchs imitiert.',
      idealFor:
        '🌸 Ideal für ein deutlicheres, strukturiertes Ergebnis mit sichtbaren "Härchen" — besonders schön bei spärlichen Brauen.',
      process:
        '📋 Ablauf: Beratung → detaillierte Vorzeichnung & Formfreigabe → Hairstroke-Pigmentierung → Pflegehinweise.',
      duration: '⏱️ Dauer: ~180 Min. Hält **1,5–3 Jahre**, verblasst sanft.',
      price: '💰 Preis: **450 €**. Nachbehandlung nach 4–8 Wochen separat.',
      healing:
        '🌿 Heilung ~4 Wochen. Härchen wirken anfangs schärfer/dunkler, werden in Woche 4 weicher. Leichte Schuppung in Woche 1 möglich.',
      aftercare:
        '✨ Gleiche Pflege: nicht berühren, keine Schüppchen entfernen, Schutz vor Sonne und Hitze.',
      compareLabel: '💖 Mit Powder Brows vergleichen',
    },
    aquarell_lips: {
      title: '💋 Aquarell Lips',
      intro:
        'Leichte, transparente Pigmentierung ohne harte Kontur — ein weicher, gleichmäßiger Ton, wie eine aufgefrischte natürliche Lippenfarbe.',
      idealFor:
        '🌸 Perfekt für ein natürliches Alltagsergebnis, zum Ausgleichen blasser Lippen oder Kaschieren leichter Asymmetrie ohne Lippenstift-Effekt.',
      process:
        '📋 Ablauf: Beratung & Farbauswahl → Vorzeichnung & Freigabe → Aquarell-Pigmentierung → Pflegehinweise.',
      duration: '⏱️ Dauer: ~130 Min. Hält **1,5–3 Jahre**, verblasst sanft.',
      price: '💰 Preis: **380 €**. Nachbehandlung nach 4–8 Wochen separat.',
      healing:
        '🌿 Heilung ~4 Wochen. In den ersten Tagen möglich: leichte Schwellung und Trockenheit, Farbe anfangs kräftiger. Endton bis Woche 4.',
      aftercare:
        '✨ Lippen gut feucht halten, keine Krusten entfernen, scharfes/heißes Essen die ersten Tage meiden.',
      compareLabel: '💄 Mit 3D Lips vergleichen',
    },
    lips_3d: {
      title: '💋 3D Lips',
      intro:
        'Sattere Pigmentierung mit visuellem Volumen — Lippen wirken voller und ausdrucksstärker, Farbe ist intensiver und stabiler.',
      idealFor:
        '🌸 Ideal, wenn Sie einen deutlichen "geschminkten" Alltags-Effekt, kräftigere Farbe und visuelles Volumen möchten.',
      process:
        '📋 Ablauf: Beratung & gesättigte Farbauswahl → detaillierte Vorzeichnung → 3D-Pigmentierung → Pflegehinweise.',
      duration: '⏱️ Dauer: ~180 Min. Hält **1,5–3 Jahre**.',
      price: '💰 Preis: **420 €**. Nachbehandlung nach 4–8 Wochen separat.',
      healing:
        '🌿 Heilung ~4 Wochen. In den ersten Tagen möglich: Schwellung und Trockenheit, Farbe intensiver. Endton bis Woche 3–4.',
      aftercare:
        '✨ Lippen gut pflegen, vorsichtig essen die ersten Tage, keine Sauna und Sonne bis vollständig verheilt.',
      compareLabel: '💋 Mit Aquarell Lips vergleichen',
    },
    lashline: {
      title: '👁 Wimpernkranz',
      intro:
        'Dezente Auffüllung zwischen den Wimpern: Der Blick wirkt ausdrucksstärker, Wimpern voller — ohne Eyeliner-Effekt.',
      idealFor:
        '🌸 Ideal, wenn Sie einen aufgefrischten Blick ohne sichtbaren Eyeliner und ohne tägliches Augen-Make-up möchten.',
      process:
        '📋 Ablauf: Beratung → Linien- & Farbauswahl → dezente Inter-Wimpern-Pigmentierung → Pflegehinweise.',
      duration: '⏱️ Dauer: ~60 Min. Hält **1,5–3 Jahre**.',
      price: '💰 Preis: **130 €**. Oben+unten — **150 €**. Nachbehandlung separat.',
      healing:
        '🌿 Heilung ~3–4 Wochen. 1–2 Tage mögliche leichte Lid-Empfindlichkeit, geringe Schwellung, kurzfristig tränende Augen.',
      aftercare:
        '✨ Augen nicht reiben, keine frischen Wimpern-Behandlungen (Extensions, Lifting) bis zur Heilung.',
      compareLabel: '👁 Mit Oben+Unten vergleichen (150 €)',
    },
    upper_lower: {
      title: '👁 Wimpernkranz Oben + Unten',
      intro:
        'Dezente Pigmentierung des Inter-Wimpern-Raums an beiden Lidern — ein deutlicheres Ergebnis und ausdrucksstärkerer Blick, weiterhin ohne Eyeliner.',
      idealFor:
        '🌸 Ideal, wenn Sie den Blick maximal auffrischen und beleben möchten, ohne tägliches Augen-Make-up.',
      process:
        '📋 Ablauf: Beratung → Linien- & Farbauswahl → Pigmentierung Oben + Unten → Pflegehinweise.',
      duration: '⏱️ Dauer: ~75 Min. Hält **1,5–3 Jahre**.',
      price: '💰 Preis: **150 €**. Nur Oben — **130 €**. Nachbehandlung separat.',
      healing:
        '🌿 Heilung ~3–4 Wochen. 1–2 Tage mögliche leichte Lid-Empfindlichkeit und geringe Schwellung.',
      aftercare:
        '✨ Augen nicht reiben, keine frischen Wimpern-Behandlungen bis vollständig verheilt, vor direkter Sonne schützen.',
      compareLabel: '👁 Mit nur Wimpernkranz vergleichen (130 €)',
    },
  };

  const N = normalized === 'ru' ? ruN : normalized === 'en' ? enN : deN;
  const n = N[technique];
  ...
  const labels =
    normalized === 'ru'
      ? {
          faq: '❓ Частые вопросы',
          healing: '🌿 Подробно о заживлении',
          preparation: '📋 Как подготовиться',
          book: '📅 Подобрать время и записаться',
        }
      : normalized === 'en'
        ? {
            faq: '❓ FAQ about PMU',
            healing: '🌿 Healing process in detail',
            preparation: '📋 How to prepare',
            book: '📅 Pick time and book',
          }
        : {
            faq: '❓ Häufige Fragen',
            healing: '🌿 Heilungsverlauf im Detail',
            preparation: '📋 So bereiten Sie sich vor',
            book: '📅 Zeit finden und buchen',
          };
  ...
  const lines = [
    ...
    n.aftercare,
    '',
    n.compareLabel ? `[option] ${n.compareLabel} [/option]` : null,
    `[option] ${labels.faq} [/option]`,
    `[option] ${labels.healing} [/option]`,
    `[option] ${labels.preparation} [/option]`,
    `[option] ${labels.book} [/option]`,
