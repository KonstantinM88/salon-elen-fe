 ...
 * @param type - тип callback: "verify" или "quick"
 */
export function getRedirectUri(type: "verify" | "quick" = "verify"): string {
  const baseUrl = getBaseUrl();
  
  if (type === "quick") {
    return `${baseUrl}/api/booking/client/google-quick/callback`;
  }
 ...
 * const authUrl = generateAuthUrl(state, "verify");
 * // Redirect пользователя на authUrl
 */
export function generateAuthUrl(state: string, type: "verify" | "quick" = "verify"): string {
...
/**
 * Alias для generateAuthUrl - более удобное имя
 */
export function getGoogleAuthUrl(state: string, type: "verify" | "quick" = "verify"): string {
  return generateAuthUrl(state, type);
}
 ...
 * 
 * @example
 * const tokenData = await exchangeCodeForToken(code, "verify");
 * const accessToken = tokenData.access_token;
 */
export async function exchangeCodeForToken(
  code: string,
  type: "verify" | "quick" = "verify"
...
export async function getUserInfo(
  ...
  try {
    ...
    // Google OAuth сам по себе является верификацией
    // Не требуем дополнительной проверки email_verified
    console.log('[Google OAuth] UserInfo received:', {
      email: data.email,
      sub: data.sub,
      id: data.id,
