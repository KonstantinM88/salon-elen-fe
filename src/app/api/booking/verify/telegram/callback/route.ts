...
async function createAppointmentFromDraft(draftId: string) {
  ...
  const appointment = await prisma.appointment.create({
    data: {
      ...
      // ✅ важно: используем enum из Prisma, а не строку "confirmed"
      status: AppointmentStatus.CONFIRMED,
    },
  });

  // Удаляем черновик после успешного создания записи
...
export async function POST(
  ...
  try {
    ...
    console.log(
      `[Telegram Callback] Payload длина: ${payload.length} символов`,
    );

    // payload = base64(JSON)
    const decoded = JSON.parse(
      Buffer.from(payload, "base64").toString("utf8"),
    ) as { d?: string; draftId?: string; e?: string; email?: string };

    console.log(`[Telegram Callback] Decoded:`, decoded);

    const draftId: string | undefined = decoded.d ?? decoded.draftId;
    let email: string | undefined = decoded.e ?? decoded.email;
    ...
    // Если email нет в payload — берём из черновика
    if (!email) {
      console.log(
        `[Telegram Callback] Email отсутствует в payload, получаем из БД...`,
      );
    ...
    console.log(
      `[Telegram Callback] Подтверждение для ${email}:${draftId}`,
    );

    // Проверяем, что OTP ещё жив
    ...
    console.log(
      `[Telegram Callback] Код подтверждён для ${email}:${draftId}`,
    );
    ...
    console.log(
      `[Telegram Callback] Appointment создан: ${appointment.id}`,
    );

    // Привязываем appointmentId к OTP, чтобы фронт мог его забрать через /status
    setAppointmentForOTP("telegram", email, draftId, appointment.id);
