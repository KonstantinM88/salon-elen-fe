...
export default function ServiceDetailModal({
  ...
  useEffect(() => {
    ...
    for (const g of service.gallery.slice(0, 5)) urls.push(g.image);

    for (const u of urls) preloadImage(u);
  }, [service.id, imageUrl, service.gallery]);

  const openGalleryFast = (index: number) => {
    // чуть-чуть прогреваем соседей, чтобы свайп был без паузы
    preloadImage(service.gallery[index]?.image);
    preloadImage(service.gallery[index + 1]?.image);
    preloadImage(service.gallery[index - 1]?.image);
    onOpenGallery(index);
  };
            ...
            <div className="p-4 sm:p-6 lg:p-8 lg:pl-6">
              ...
              {service.description && (
                <div className="relative mb-6 lg:mb-8 overflow-hidden">
                  <p className="relative z-10 text-zinc-600 dark:text-rose-100/80 text-sm sm:text-base lg:text-lg leading-relaxed whitespace-pre-line font-cormorant font-medium">
                    {service.description}
                  </p>
                </div>
              )}
...
//------пробуем ускорить показ галереи-------
// // src/app/services/ServiceDetailModal.tsx
// "use client";
...
//--------пробуем ускорить загрузку главного банера-------
// // src/app/services/ServiceDetailModal.tsx
// "use client";
