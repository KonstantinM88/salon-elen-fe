...
function estimateReadingMinutes(text: string | null | undefined): number {
  if (!text) return 1;
  ...
  const words = plainText.match(/[\p{L}\p{N}]+(?:['\p{Pd}][\p{L}\p{N}]+)*/gu) ?? [];
...
export default async function Page({
  ...
  const readMinutes = estimateReadingMinutes(content || excerpt || title);
  ...
  return (
    <main className="px-4">
      ...
      <article className="mx-auto max-w-3xl py-8">
        ...
        <NewsImageCarousel images={articleImages} title={title} locale={locale} />
        ...
        {content && <MarkdownContent body={content} />}
      </article>
    </main>
  );
}
