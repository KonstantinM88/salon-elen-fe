...
export default function PaymentPageClient(): React.JSX.Element {
  ...
  const handleAddToAppleCalendar = async () => {
    try {
      ...
      if (isIOS) {
        // ✅ iOS FIX: Используем серверный endpoint
        // Safari правильно обработает .ics файл с сервера
        const calendarUrl = `/api/appointments/${appointmentId}/calendar?locale=${locale}`;
        
        console.log('📅 iOS: Redirecting to calendar endpoint:', calendarUrl);
        
        // Перенаправляем на серверный endpoint
        // Safari скачает файл и предложит открыть в Calendar
        window.location.href = calendarUrl;
        
        console.log('📅 iOS: Redirect initiated');
        
      } else {
        // Desktop/Android: Создаём файл на клиенте
        console.log('📅 Desktop/Android: Creating .ics file...');
        ...
        console.log('📅 Desktop/Android: File downloaded');
      }
...
//---------для андроид работает для айос не реагирует, меняем с новым роутером-------
// // src/app/booking/payment/PaymentPageClient.tsx
// "use client";
...
//       if (isIOS) {
//         // ✅ iOS FIX: Используем data URL + window.open
//         // Safari открывает в новой вкладке и предлагает скачать/открыть
...
//         console.log('📅 iOS: Opening data URL in new window...');
        
//         // Открываем в новой вкладке - Safari покажет диалог "Открыть в Calendar"
//         window.open(dataUrl, '_blank');
        
//         console.log('📅 iOS: Data URL opened');
        
//       } else {
//         // Desktop/Android: Скачиваем файл как обычно
//         const icsBlob = new Blob([icsContent], { type: 'text/calendar;charset=utf-8' });
//         const url = window.URL.createObjectURL(icsBlob);
...
//-----------пытаюсь исправить Apple calendar-----
// // src/app/booking/payment/PaymentPageClient.tsx
// "use client";
...
//       // ✅ iOS FIX: Определяем iOS
//       const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
      
//       if (isIOS) {
//         // ✅ iOS: Открываем серверный endpoint напрямую
//         // Safari распознает MIME тип text/calendar и предложит добавить в Calendar
//         const calendarUrl = `/api/appointments/${appointmentId}/calendar?locale=${locale}`;
        
//         console.log('📅 iOS: Opening calendar endpoint:', calendarUrl);
        
//         // Открываем в текущей вкладке - Safari откроет Calendar app
//         window.location.href = calendarUrl;
        
//       } else {
//         // Desktop/Android: Создаём .ics файл на клиенте и скачиваем
//         const response = await fetch(`/api/appointments/${appointmentId}`);
...
//--------------фиксю нажатие кнопок календаря на  iOS Safari -------
// // src/app/booking/payment/PaymentPageClient.tsx
// "use client";
...
//--------------добавляю иконки для календарей------
// // src/app/booking/payment/PaymentPageClient.tsx
// "use client";
...
//------------фиксим баг с языком гугл календаря---------
// // src/app/booking/payment/PaymentPageClient.tsx
// "use client";
...
//---------исправляем адвативность для мобильных устройств------
// // src/app/booking/payment/PaymentPageClient.tsx
// "use client";
