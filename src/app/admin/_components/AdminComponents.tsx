// src/app/admin/_components/AdminComponents.tsx
// Адаптивные компоненты для admin/page.tsx — Light + Dark theme
...
export function KPICard({
  ...
  const tones = {
    sky: {
      ...
      bg: "from-sky-500/5 dark:from-sky-500/10 to-sky-500/0",
    },
    emerald: {
      ring: "ring-emerald-400/30",
      icon: "text-emerald-600 dark:text-emerald-400",
      bg: "from-emerald-500/5 dark:from-emerald-500/10 to-emerald-500/0",
    },
    violet: {
      ring: "ring-violet-400/30",
      icon: "text-violet-600 dark:text-violet-300",
      bg: "from-violet-500/5 dark:from-violet-500/10 to-violet-500/0",
    },
    rose: {
      ring: "ring-rose-400/30",
      icon: "text-rose-600 dark:text-rose-300",
      bg: "from-rose-500/5 dark:from-rose-500/10 to-rose-500/0",
    },
  }[tone];
  ...
  return (
    <div
      className="card-glass card-glow p-4 sm:p-5 transition-all duration-300
                 hover:-translate-y-0.5 hover:border-gray-300 dark:hover:border-white/20"
    >
      <div className={`absolute inset-0 bg-gradient-to-br ${tones.bg}`} />
      <div className="relative flex items-start gap-3">
        <IconGlow
          tone={tone}
          className={`h-10 w-10 sm:h-12 sm:w-12 ring-2 ${tones.ring} bg-gray-50 dark:bg-white/5 shrink-0`}
        >
          <span className={tones.icon}>{icon}</span>
        </IconGlow>
        <div className="grow min-w-0">
          <div className="text-xs sm:text-sm text-gray-500 dark:text-white/70 mb-1">{title}</div>
          <div className="text-xl sm:text-2xl lg:text-3xl font-semibold text-gray-900 dark:text-white truncate">
            {value}
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <IconGlow
          tone={color}
          className={`h-10 w-10 ring-2 ${ringMap} bg-gray-50 dark:bg-white/5 shrink-0`}
        >
          {icon}
        </IconGlow>
        <div className="text-base font-medium text-gray-900 dark:text-white truncate">{title}</div>
      </div>
      <p className="text-sm text-gray-500 dark:text-white/70 line-clamp-2">{description}</p>
      <Link
        href={href}
...
export function SectionWrapper({
  ...
  return (
    <section className={`space-y-4 ${className}`}>
      {title && (
        <h2 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-white/90">{title}</h2>
      )}
      {children}
    </section>
  );
...
/* ───────── TABLE WRAPPER - Обёртка для таблиц ───────── */
export function TableWrapper({ children }: { children: ReactNode }) {
  return (
    <div className="overflow-x-auto -mx-4 sm:mx-0">
      <div className="inline-block min-w-full align-middle">
        <div className="overflow-hidden border border-gray-200 dark:border-white/5 rounded-xl">
          {children}
        </div>
      </div>
    </div>
    ...
    >
      <div className="flex items-start gap-3">
        {icon && (
          <IconGlow
            tone={tone}
            className="h-8 w-8 bg-gray-50 dark:bg-white/5 ring-1 ring-gray-200 dark:ring-white/10 shrink-0"
          >
            {icon}
          </IconGlow>
        )}
        <div className="grow min-w-0">
          <div className="text-xs text-gray-500 dark:text-white/70 mb-0.5">{title}</div>
          <div className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white truncate">{value}</div>
          {subtitle && (
            <div className="text-xs text-gray-400 dark:text-white/60 mt-0.5 truncate">{subtitle}</div>
          )}
        </div>
      </div>
    </div>
...
//---------19.02.27 адаптируем под светлую тему
